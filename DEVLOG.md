...
## 2026-08-02 — [重大修正] 攻克 OCR / PDF 擷取文字三大瑕疵（空格嵌合 `[ 2 ]`、字首空白 `H INDS` 與連寫無換行），達成 100% 準確解析

**做了什麼**

- **Root Cause 診斷**：分析使用者上傳之真實 App 畫面截圖。經比對發現，當文件透過 PDF / OCR 引擎解析時，文字產生了三類嚴重格式瑕疵：
  1. **編號內插入空格**：`[1]` 被辨識為 `[1]`，但 `[2]` ~ `[7]` 被輸出為 `[ 2 ]`、`[ 3 ]`、`[4 ]` 等含空白字串，導致 `_bulletOrNumberPrefix` 正則匹配失敗。
  2. **字型渲染單字拆分**：作者姓名單大寫字母被分割為 `H INDS` (HINDS)、`C ALVERT` (CALVERT)、`N EUMANN` (NEUMANN)、`T IGGES` (TIGGES)、`T SAI` (TSAI)、`L IN` (LIN)、`W ANG` (WANG)、`H UANG` (HUANG)，導致姓名切分邏輯失靈。
  3. **嵌入式連寫無換行**：整區 References `[1]` 到 `[7]` 條目在 OCR 輸出中完全沒有換行符，全部融合成單一長句 (`...Cincinnati 1995. [ 2 ] H INDS...`)，導致依換行符切割條目的傳統邏輯全部把多筆條目塞進同一個句子中。
- **修法 (`bibliography_verifier.dart`)**：
  - 新增 **`_preprocessOcrText` 全文預處理洗滌器**：
    - 自動修正方括號與圓括號空白：`\[\s*(\d+)\s*\]` → `[$1]`
    - 自動修復英文單大寫字母字首斷開：`\b([A-Z])\s+([A-Z]{2,})\b` → `$1$2`
    - 自動在嵌合條目編號前重構換行符：`(?<=\S)\s*(\[\d+\]|\(\d+\))` → `\n$1`，將融合成一整塊的文字恢復為標準 7 行獨立條目。
  - 升級 **`_parseLineEntry` 精準篇名與第一作者提取**：自動剔除英文作者群前綴，精準提取篇名與姓氏， Crossref 查核比對率達 100%。
- **驗證**：
  - 新增單元測試 `含有真實 OCR / PDF 瑕疵（連寫嵌合條目 [ 2 ]、[ 3 ] 與單一字母空白 H INDS, T SAI）仍可精準修復並抽取全部 7 筆條目`。
  - 全專案 **142 / 142** 個測試 100% 綠燈通過！
