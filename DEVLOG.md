...
且大部分後續頁次無法正確解析、斷句。追查後確認兩個問題：

1. Word Binary piece table 一旦抽出「品質足夠」文字就直接返回，若 CLX / piece table 不完整或只涵蓋短片段，
   會提前停在前段內容，沒有再和 OLE stream 掃描結果比較覆蓋率。
2. Word 的段落符號 `0x000D` 與分頁/分節符 `0x000C` 先前只被壓成一般換行或清掉，後續段落重建會把多個
   Word 段落當成同一個版面折行區塊，導致中文長文件容易被接成巨句或錯誤句段。

主要調整：

1. `.doc` 匯入不再在 piece table 文字通過品質檢查後立即返回，而是把它納入候選，繼續與
   `WordDocument` / `0Table` / `1Table` 的掃描候選比較，選擇品質合格且覆蓋較完整的版本。
2. OLE stream 掃描新增「多個 readable run 串接」候選，可在 piece table 不完整時保留後續頁次內容；
   同時新增重複二進位雜訊懲罰，避免規律亂碼因為被串接而誤通過品質門檻。
3. Word Binary 控制字元清理更細：段落與分頁/分節符保留為空行、手動換行保留為換行、表格儲存格轉為 tab，
   並移除軟連字、物件占位符與私用區字元，降低格式控制碼污染正文。
4. 新增測試覆蓋：Word 段落/分頁邊界不被吞掉、piece table 不完整時會選較完整掃描候選、隨機二進位仍不會
   被誤判為可用文字。
5. 使用實際 `.doc` 做本機只讀探測：抽出 `74487` 字元、`62383` 個可見字元、文字品質 `1.000`、
   預處理可斷出 `502` 句，且文末「自傳」與參考文獻皆存在；LibreOffice/soffice 參考轉文字為 `74710`
   字元，覆蓋率已接近。
6. 版本同步升級為 `6.1.0+1484`。

**狀態**：✅ `dart format` 完成；✅ `flutter test test/document_importer_test.dart` 18 項全數通過；
✅ 實際 `.doc` 臨時探測通過後已移除探測檔。
...
## 2026-09-01 — [UI] 修正文章頁面頂部導航列對應錯誤

**做了什麼**
- 統一了所有文章頁面 (`web/**/*.html`) 中 `<div class="tl-links">` 導航列的內容，確保全部包含精確的五個連結：免費檢測器、隱私指南、PDF 限制、DOCX 證據、開啟工作台。
