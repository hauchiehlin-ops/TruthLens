...
class AppLocalizationsZh extends AppLocalizations {
  ...
  String get helpRisksBody =>
      '以下每一項都是本工具真實存在的限制，請在做出任何決定前一併考慮：\n\n1. 來源證據可以被清除或偽造\n另存新檔、線上轉檔、從 Google 文件匯出、或複製到新檔案，都會讓編輯紀錄歸零。因此有訊號只是佐證，沒有訊號也絕不代表文件必然由人撰寫。\n\n2. 共形保證依賴「可交換性」\n保證成立的前提是基準樣本與待測文章出自同一群人、同一類寫作任務。學生寫作能力明顯進步、或換了完全不同的題型，前提就不再成立，需要重建基準集。\n\n3. 基準集本身可能被汙染\n如果拿來當基準的作業其實是 AI 代寫的，整個校準都會偏掉。基準樣本必須在可控環境下蒐集，例如課堂當場完成的作業。\n\n4. 瀏覽器端小模型的原始準度不如伺服器端大模型\n這是 Web-only 決策換取隱私的必然代價。本工具的價值不在單一分數更準，而在可解釋、可校準、且會誠實棄權。\n\n5. 任何分數都不應單獨作為指控的依據\n請務必搭配逐句證據、文件來源，以及你對這名學生既有的了解一起判讀。本工具的設計目標是輔助你進行對話，不是代替你做出裁決。';
...
class AppLocalizationsZhHans extends AppLocalizationsZh {
  ...
  @override
  String get helpRisksBody =>
      '以下每一项都是本工具真实存在的限制，请在做出任何决定前一并考虑：\n\n1. 来源证据可以被清除或伪造\n另存新档、在线转档、从 Google 文档导出、或复制到新文件，都会让编辑记录归零。因此有信号只是佐证，没有信号也绝不代表文件必然由人撰写。\n\n2. 共形保证依赖「可交换性」\n保证成立的前提是基准样本与待测文章出自同一群人、同一类写作任务。学生写作能力明显进步、或换了完全不同的题型，前提就不再成立，需要重建基准集。\n\n3. 基准集本身可能被污染\n如果拿来当基准的作业其实是 AI 代写的，整个校准都会偏掉。基准样本必须在可控环境下收集，例如课堂当场完成的作业。\n\n4. 浏览器端小模型的原始准度不如服务器端大模型\n这是 Web-only 决策换取隐私的必然代价。本工具的价值不在单一分数更准，而在可解释、可校准、且会诚实弃权。\n\n5. 任何分数都不应单独作为指控的依据\n请务必搭配逐句证据、文件来源，以及你对这名学生既有的了解一起判读。本工具的设计目标是辅助你进行对话，不是代替你做出裁决。';
