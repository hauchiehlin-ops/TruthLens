...
void main() {
  ...
  test('匯入模型後完整分析：Transformer 引擎參與且產出逐句分數', () async {
    ...
    final modelFile = File(p.join(support.path, 'models', 'verify_model.onnx'));
    final tokFile = File(
    ...
    final transformer = result.engineScores.firstWhere(
      (s) => s.engineId == 'transformer',
    );
    expect(
    ...
    // 清理：移除測試匯入的變體
    await mm.removeVariant(
      'transformer',
      mm.activeVariant('transformer')!.variantId,
    );
    // ignore: avoid_print
    print(
      '完整分析 → 整體 AI:${result.aiProbability} '
