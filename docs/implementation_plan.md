...
> **目標**：打造一款超越現有市場的、本地優先運算的跨平台 AI 內容檢測應用程式
...
| 原則 | 說明 |
| :--- | :--- |
| **本地優先＋必要連線** | 核心 AI 推論與文件內容一律在裝置端處理、不上傳；模型更新偵測與超連結／期刊文獻目錄驗證等主動分析功能會視需要連線，僅傳送版本號、網址或 DOI（2026-07-05 起由「完全離線」調整，理由見 DEVLOG） |
...
- 文件匯入、OCR／貼上、四引擎即時進度、初步分數、逐句證據與完整報告共用同一份分析狀態，不以頁面跳轉切斷流程
- 原始輸入版面保留為預設首頁；使用者可改用自動、指揮網格、任務時間軸或證據畫布，所有選擇均保存於本機
- 提供「指揮網格」、「任務時間軸」與「證據畫布」三種布局；分析進行中可直接切換，不重啟推論
- 自動模式於桌面採指揮網格、行動裝置採任務時間軸；使用者選擇會保存在本機偏好設定
- 完整報告支援嵌入工作台，仍保留逐句熱區、引擎貢獻、連結／文獻核實及 PDF／CSV／JSON／PNG 匯出
- 所有模式共用相同判定資料、顏色語意與多國語系文案，避免布局造成結果不一致
- 外觀預設跟隨作業系統亮／暗模式，亦可手動固定；面板、輸入區與判定狀態色使用亮度感知的 Material 語意色
...
| 模型大小 | ~120MB（量化後） |
| 分析粒度 | 段落內受控區塊（最多 5 句／約 120 詞元），區塊分數映射回逐句證據 |
