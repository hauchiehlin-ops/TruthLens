...
- 原始輸入版面保留為可選首頁；新安裝預設為「自動選擇」，桌面採指揮網格、行動裝置採任務時間軸，所有選擇均保存於本機
- 分析期間可即時切換三種工作台配置；原始版面會暫時停用以避免卸載進行中的分析狀態。工作台持續顯示經過時間、已完成與執行中的模組，長時間未完成模組時會提示分析仍在進行
- 原生 ONNX 推論批次之間主動讓出事件迴圈，確保分析動畫、計時及工作台切換能持續重繪
- 原始首頁與各工作台共用品牌／版本標題及工作台切換器；分析遙測以四段環形總覽、桌面狀態軌道與行動版脈衝節點呈現四引擎生命週期，流程完成度與 AI 判定機率採不同顯示時機
- 主要分析動作依生命週期切換為「開始分析／停止分析／新的分析」；停止分析前必須確認，保留目前文件文字、清除未完成遙測且不建立歷史紀錄。已取消執行的延遲回呼與原生推論結果一律丟棄，不得覆寫目前工作台
...
> 作者判讀融合、三方向輸出與固定 FPR 發布閘門的現行規格，詳見
> [TruthLens vNext 作者判讀設計](detection_vnext.md)。本節早期的「四引擎加權投票」
> 僅代表模型配置，不再代表直接平均分數的實作方式。
